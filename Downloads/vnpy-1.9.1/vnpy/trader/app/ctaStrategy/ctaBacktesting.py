...
class BacktestingEngine(object):
    ...
    def __init__(self):
        ...
        self.capital = 0      # 回测时的起始本金（默认100万）
    ...
    # 数据回放相关
    #------------------------------------------------    
    def isoStr2utc8Str(self,isoStr):
    ...
    def loadHistoryData(self):
        ...
        if self.hdsClient:
            ...
            if self.dbName != 'Matrixdata_bar':
                flt = {'datetime':{'$gte':self.dataStartDate,
                                   '$lt':self.strategyStartDate}}
                initCursor = collection.find(flt).sort('datetime')
            else:
        		    # 载入初始化需要用的数据
                flt = {'Time':{'$gte':self.dataStartDate.isoformat(),
                               '$lt':self.strategyStartDate.isoformat()}}
                initCursor = collection.find(flt).sort('Time')
        ...
        for d in initCursor:
            if self.dbName == 'Matrixdata_bar':
                ...
                d['volume'] = float(d['QuoteVolume'])
                d['vtSymbol'] = (d['Symbol'])
                d['symbol'] = (d['Symbol'])
                d['exchange'] = (d['Symbol'])

                d['datetime'] = self.isoStr2utc8Str(d['Time'])
        ...
        if self.hdsClient:
            ...
            if self.dbName == 'Matrixdata_bar':
                if not self.dataEndDate:
                    flt = {'Time':{'$gte':self.strategyStartDate.isoformat()},'QuoteVolume':{'$ne':u'0'}}   # 数据过滤条件
                else:
                    flt = {'Time':{'$gte':self.strategyStartDate.isoformat(),
                                   '$lte':self.dataEndDate.isoformat()},'QuoteVolume':{'$ne':u'0'}}
                self.dbCursor = collection.find(flt).sort('Time')
            else:
                if not self.dataEndDate:
    ...
    def calculateBacktestingResult(self):
        ...
        self.initCapital = self.tradeDict['1'].price * 1 * 1
        ...
        for result in resultList:
            ...
            if maxCapital == 0:
                ...
                drawdownRate = abs(drawdown/maxCapital)*100
        ...
        # 返回回测结果
        d = {}
        d['capital'] = capital - self.initCapital
        d['initcapital'] = self.initCapital
        d['profitrate'] = (capital - self.initCapital)/self.initCapital *100
    ...
    def showBacktestingResult(self):
        ...
        self.output(u'初始金额：\t%s' % formatNumber(d['initcapital']))
        self.output(u'收益率：\t%s%%' % formatNumber(d['profitrate']))


        self.output(u'最大回撤: \t%s' % formatNumber(min(d['drawdownList'])))
        self.output(u'最大回撤率: \t%s%%' % formatNumber(max(d['drawdownRateList'])))
