...
class CtaEngine(AppEngine):
    ...
    def sendOrder(self, vtSymbol, orderType, price, volume, strategy):
        ...
        self.writeCtaLog(u'策略%s发送委托，%s，%s，%s，%d，%s@%s'
                         %(strategy.name, vtSymbol, req.direction,req.offset, req.tradetype, volume, price))
    ...
    def cancelOrder(self, vtOrderID):
        ...
        if order:
            ...
            if not orderFinished:
                ...
                str = order.symbol.split('.')
                if len(str) > 2:
                    req.symbol = str[0]
                    req.contracttype = str[1]
                else:
    ...
    def processPositionEvent(self, event):
        ...
        #print("symbol %s, position %f, eveningup %f" % (pos.symbol,pos.position,pos.position-pos.frozen))
    ...
    def addtickStrategy(self, strategy, vtSymbol):
    ...
    def initStrategy(self, name):
        ...
        if name in self.strategyDict:
            ...
            if not strategy.inited:
                ...
                self.subscribeMarketData(strategy.vtSymbol)                      # 加载同步数据后再订阅行情
            else:
        ...
        for orderID in list(s):
            if orderID != None:
                if STOPORDERPREFIX in orderID:
                    print("Cancel Stop order %s" % orderID)
                    self.cancelStopOrder(orderID)

                else:
                    print("Cancel order %s" % orderID)
